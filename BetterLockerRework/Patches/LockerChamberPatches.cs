	...
	public static bool Prefix(Locker __instance, LockerChamber ch)
	{
		...
		if (cfg.LockerSpawns.TryGetValue(__instance.StructureType, out var spawnerList) && spawnerList.Count > 0)
		{
			...
			foreach (var spawner in shuffledList)
			{
				...
				{
					if (cfg.DebugMode)
						Log.Info($"Skipped {spawner.item} in {__instance.StructureType}: MaxAmountInLocker {spawner.maxamountinlocker} reached.");
					continue;
				}
				...
				if (UnityEngine.Random.Range(1, 101) <= spawner.chance)
				{
					if (!spawner.uciitem)
					{
						...
						{
							Log.Warn($"Invalid ItemType '{spawner.item}' for locker {__instance.StructureType}");
						}
					}
					else if (uciUtilsType != null && summonedCustomItemType != null)
					{
						...
						{
							Log.Warn($"CustomItem '{spawner.item}' not found.");
							continue;
						}
					...
					{
						Log.Warn($"UCI not present: Skipping custom item '{spawner.item}'");
					}
				}
			}
